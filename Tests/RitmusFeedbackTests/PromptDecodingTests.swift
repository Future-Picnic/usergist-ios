...
final class PromptDecodingTests: XCTestCase {
    ...
    func test_decodesAllFourQuestionTypes() throws {
        ...
        let decoded = try JSONDecoder.ritmus().decode(ArmedTriggersResponse.self, from: data)
    ...
    func test_roundTripEncodeDecode() throws {
        let data = fixtureJSON.data(using: .utf8)!
        let decoded = try JSONDecoder.ritmus().decode(ArmedTriggersResponse.self, from: data)
        let reencoded = try JSONEncoder.ritmus().encode(decoded)
        let redecoded = try JSONDecoder.ritmus().decode(ArmedTriggersResponse.self, from: reencoded)
