...
final class Runtime {
    ...
    init(config: UserGistConfig) throws {
        ...
        self.userStateTracker = UserStateTracker(queue: UserGistQueue.serial("user-state"))
        ...
        apiClient.getSurveyFlow(surveyId: surveyId, language: language) { [weak self] result in
            ...
            case .failure(let err):
                ...
                DispatchQueue.main.async {
                    UserGist.shared.surveyHandlers.onShow?(surveyId)
                }
            }
        }
    }
        ...
        DispatchQueue.main.async {
            if #available(iOS 14.0, *) {
                SurveyHost.present(
                    ...
                    handlers: UserGist.shared.surveyHandlers,
                    resume: resume,
                    logger: self.logger
                )
            } else {
