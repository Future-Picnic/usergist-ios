...
/// Notification Service Extension for UserGist pushes.
///
/// Host apps add a new "Notification Service Extension" target in
/// Xcode and depend on the `UserGistFeedbackNSE` product. They subclass
...
///      `usergist.deliveryId` so we record true delivered_at, distinct
...
open class UserGistNotificationService: UNNotificationServiceExtension {
    ...
    open override func didReceive(
        ...
        // Rich media attachment if `usergist.extra.imageUrl` is present.
    ...
    private func isSilentPing(_ userInfo: [AnyHashable: Any]) -> Bool {
        guard let silent = userInfo["usergist_silent"] as? String else { return false }
        return silent == "1"
    }

    private func imageUrlFrom(userInfo: [AnyHashable: Any]) -> URL? {
        guard
            let usergist = userInfo["usergist"] as? [String: Any],
    ...
    private func pingIdFrom(userInfo: [AnyHashable: Any]) -> String? {
        return userInfo["usergist_ping_id"] as? String
    }

    private func beaconDelivered(userInfo: [AnyHashable: Any]) {
        guard let deliveryId = deliveryIdFrom(userInfo: userInfo) else { return }
        guard let writeKey = UserGistNSEConfig.writeKey, let apiUrl = UserGistNSEConfig.apiUrl else {
            return
        }
    ...
    private func ackSilent(userInfo: [AnyHashable: Any], completion: @escaping () -> Void) {
        ...
        guard let writeKey = UserGistNSEConfig.writeKey,
              ...
              let anonymousId = UserGistNSEConfig.anonymousId else {
            return completion()
        }
    ...
    private func downloadAttachment(
        ...
        let task = URLSession.shared.downloadTask(with: url) { tempUrl, response, _ in
            ...
            do {
                ...
                let attachment = try UNNotificationAttachment(identifier: "usergist-image", url: dst)
