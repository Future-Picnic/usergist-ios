...
final class TLSPinnedSessionDelegate: NSObject, URLSessionDelegate {
    ...
    init(pinSets: [TLSPinSet], logger: RitmusLogger) {
...
enum TLSPinEnv {
    static let leaf = "RITMUS_TLS_PIN_LEAF"
    static let backup = "RITMUS_TLS_PIN_BACKUP"
    static let pinnedHost = "api.ritmus.studio"
}

extension RitmusConfig {
    /// Pin sets used by the SDK's URLSession. Defaults to pinning
    /// `api.ritmus.studio` against the two production SPKI hashes; set
