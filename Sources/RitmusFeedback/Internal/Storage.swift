...
/// `Library/Caches/Ritmus/{writeKeyHash}/`.
///
...
final class Storage {
    ...
    init(writeKeyHash: String, fileManager: FileManager = .default) throws {
        ...
        let root = caches
            .appendingPathComponent("Ritmus", isDirectory: true)
    ...
    func readJSON<T: Decodable>(_ type: T.Type, at url: URL) throws -> T? {
        ...
        return try JSONDecoder.ritmus().decode(type, from: data)
    }

    func writeJSON<T: Encodable>(_ value: T, to url: URL) throws {
        let data = try JSONEncoder.ritmus().encode(value)
