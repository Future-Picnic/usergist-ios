...
///
/// Created from the arguments to `UserGist.initialize`. Everything the SDK
/// needs at runtime (API URL, timings, limits) comes from here — no
/// hidden singletons reading back into this.
struct UserGistConfig: Equatable {
    let writeKey: String
    ...
    init(
        ...
        sdkVersion: String = UserGistConfig.currentSDKVersion,
        maxQueueBytes: Int = UserGistConfig.defaultMaxQueueBytes
    ) {
